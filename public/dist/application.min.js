...
"use strict";angular.module("contents").controller("ContentsController",["$scope","$window","$stateParams","$location","Authentication","Contents","$rootScope",function($scope,$window,$stateParams,$location,Authentication,Contents,$rootScope){$scope.authentication=Authentication,$scope.tile_limit=120,$scope.rowLength=4,$scope.ytChat="https://www.youtube.com/live_chat?v="+$window.youtubeStreamID+"&embed_domain=www.seoulcommunityradio.com",$scope.ytVideo="https://www.youtube.com/embed/"+$window.youtubeStreamID,$scope.removeContent=function(content){if(content){content.$remove();for(var i in $scope.contents)$scope.contents[i]===content&&$scope.contents.splice(i,1)}else $scope.content.$remove(function(){$location.path("contents")})},$rootScope.changeBg($scope.defaultBg),$scope.find=function(){var query=$stateParams.contentType?{category:$stateParams.contentType}:{};$scope.contentType=$stateParams.contentType,$scope.contents=Contents.query(query,function(data){$scope.changeBg($scope.defaultBg)})},$scope.findOne=function(){console.log($stateParams.contentId),$scope.content=Contents.get({contentId:$stateParams.contentId},function(data){console.log(data),$scope.changeBg(data.image)})}}]);
...
"use strict";angular.module("core").filter("trustUrl",["$sce",function($sce){return function(url){return $sce.trustAsResourceUrl(url)}}]);
