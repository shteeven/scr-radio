...
"use strict";angular.module("things").run(["Menus",function(Menus){Menus.addMenuItem("topbar",{title:"Things",state:"things",type:"dropdown",roles:["*"]}),Menus.addSubMenuItem("topbar","things",{title:"Create Things",state:"things.create",roles:["user"]})}]);
"use strict";angular.module("things").config(["$stateProvider",function($stateProvider){$stateProvider.state("things",{"abstract":!0,url:"/things",template:'<div class="view"><ui-view/></div>'}).state("things.list",{url:"",templateUrl:"modules/things/client/views/list-things.client.view.html"}).state("things.create",{url:"/create",templateUrl:"modules/things/client/views/create-thing.client.view.html",data:{roles:["user","admin"]}}).state("things.view",{url:"/:thingId",templateUrl:"modules/things/client/views/view-thing.client.view.html"}).state("things.edit",{url:"/:thingId/edit",templateUrl:"modules/things/client/views/edit-thing.client.view.html",data:{roles:["user","admin"]}})}]);
"use strict";angular.module("things").controller("ThingsController",["$scope","$stateParams","$location","Authentication","Things","Specials","Episodes",function($scope,$stateParams,$location,Authentication,Things,Specials,Episodes){$scope.authentication=Authentication,$scope.clear=function(){$scope.thing.title="",$scope.thing.image="",$scope.thing.images=[],$scope.thing.links={},$scope.thing.categories=[],$scope.thing.description={},$scope.thing.guest=!1,$scope.thing.featured=!1},$scope.create=function(isValid){if($scope.error=null,!isValid)return $scope.$broadcast("show-errors-check-validity","thingForm"),!1;var thing=new Things({title:$scope.thing.title,image:$scope.thing.image,images:$scope.thing.images,links:$scope.thing.links,categories:$scope.thing.categories,description:$scope.thing.description,guest:$scope.thing.guest,featured:$scope.thing.featured});thing.$save(function(response){$location.path("things/"+response._id),$scope.clear()},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.remove=function(thing){if(thing){thing.$remove();for(var i in $scope.things)$scope.things[i]===thing&&$scope.things.splice(i,1)}else $scope.thing.$remove(function(){$location.path("things")})},$scope.update=function(isValid){if($scope.error=null,!isValid)return $scope.$broadcast("show-errors-check-validity","thingForm"),!1;var thing=$scope.thing;thing.$update(function(){$location.path("things/"+thing._id)},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.find=function(){$scope.things=Things.query({},function(data){var item=data[Math.floor(Math.random()*data.length)];$scope.changeBg(item.image)})},$scope.findOne=function(){$scope.thing=Things.get({thingId:$stateParams.thingId},function(data){$scope.changeBg(data.image)}),$scope.specials=Specials.query({thingId:$stateParams.thingId,limit:5}),$scope.episodes=Episodes.query({thingId:$stateParams.thingId,limit:10})}}]);
