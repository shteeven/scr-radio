...
"use strict";angular.module("core").directive("audioPlayer",["$rootScope","$document","$timeout","$http",function($rootScope,$document,$timeout,$http){return{restrict:"E",scope:{},link:function($scope,$element){function getMetadata(){$http.jsonp("https://seoulcommunityradio.airtime.pro/api/live-info-v2",{params:{format:"jsonp",callback:"JSON_CALLBACK"}}).then(function(data){if(data=data.data,console.log(data.tracks),$scope.metadata&&data.tracks&&data.tracks.current&&data.tracks.current.name===$scope.metadata.tracks.current.name)timer=1e4,setTimer(timer);else{$scope.metadata=data,timeNow=new Date(data.station.schedulerTime);var endDateTime=data.tracks.next?data.tracks.next.starts:data.tracks.current.ends;timeEnd=new Date(endDateTime),timer=timeEnd.getTime()-timeNow.getTime()+100,setTimer(timer)}},function(err){console.log(err)})}function setTimer(interval){$timeout.cancel(timeout),timeout=$timeout(function(){getMetadata()},interval)}var timeout,timeNow,timeEnd,timer=50,radio=$document[0].createElement("audio");radio.src="http://seoulcommunityradio.out.airtime.pro:8000/seoulcommunityradio_a?1466567397161.mp3";var player={};player.isPlaying=!1,player.volumeChange=function(direction){1!==radio.volume&&"up"===direction?(radio.volume=radio.volume+.1,$scope.opacityPlus={opacity:1-.6*radio.volume},$scope.opacityMinus={opacity:.4+.6*radio.volume}):0!==radio.volume&&(radio.volume=radio.volume-.1,$scope.opacityPlus={opacity:1-.6*radio.volume},$scope.opacityMinus={opacity:.4+.6*radio.volume}),radio.volume>=.999?($scope.atMax=!0,radio.volume=1):radio.volume<=.001?($scope.atMin=!0,radio.volume=0):($scope.atMax=!1,$scope.atMin=!1)},player.playPause=function(){player.isPlaying?(radio.pause(),$timeout.cancel(timeout),player.isPlaying=!1):(radio.play(),player.isPlaying=!0,$rootScope.$broadcast("audio.play",this))},$scope.$on("player.play",function(){radio.pause()}),$scope.player=player,radio.volume=.5,$scope.opacityPlus={opacity:1-.6*radio.volume},$scope.opacityMinus={opacity:.4+.6*radio.volume},setTimer(10)},templateUrl:"modules/core/client/views/components/audio-player.html"}}]);
"use strict";var app=angular.module("core");app.directive("scrCarousel",["$rootScope","$http","$interval","$state",function($rootScope,$http,$interval,$state){return{restrict:"E",scope:{slides:"="},controller:["$scope","$element",function($scope,$element){$scope.currentIndex=0,$scope.go=function(slide){"576b22cf2c06f24eb4f3eea0"===slide._id?$state.go("contents.livestream"):$state.go("contents.view",{contentId:slide._id})};var rotationDelay=68e4;$scope.isCurrentSlideIndex=function(index){return $scope.currentIndex===index},$scope.direction="slide-right",$scope.next=function(){$scope.direction="slide-left",$scope.currentIndex=$scope.currentIndex>0?--$scope.currentIndex:$scope.slides.length-1},$scope.previous=function(){$scope.direction="slide-right",$scope.currentIndex=$scope.currentIndex<$scope.slides.length-1?++$scope.currentIndex:0};var rotateSlide=$interval(function(){$scope.next()},rotationDelay);$scope.startRotation=function(){rotateSlide&&($interval.cancel(rotateSlide),rotateSlide=void 0),rotateSlide=$interval(function(){$scope.next()},rotationDelay)},$scope.stopRotation=function(){rotateSlide&&($interval.cancel(rotateSlide),rotateSlide=void 0)},$scope.$on("$destroy",function(){rotateSlide&&($interval.cancel(rotateSlide),rotateSlide=void 0)})}],templateUrl:"modules/core/client/views/components/carousel.html"}}]);
