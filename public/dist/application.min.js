...
"use strict";ApplicationConfiguration.registerModule("djs",["core"]),ApplicationConfiguration.registerModule("djs.admin",["core.admin"]),ApplicationConfiguration.registerModule("djs.admin.routes",["core.admin.routes"]);
...
"use strict";angular.module("djs").run(["Menus",function(Menus){Menus.addMenuItem("topbar",{title:"Djs",state:"djs",type:"dropdown",roles:["*"]}),Menus.addSubMenuItem("topbar","djs",{title:"Residents",state:"djs.list"}),Menus.addSubMenuItem("topbar","djs",{title:"Create Djs",state:"djs.create",roles:["user"]})}]);
"use strict";angular.module("djs").config(["$stateProvider",function($stateProvider){$stateProvider.state("djs",{"abstract":!0,url:"/djs",template:'<div class="view"><ui-view/></div>'}).state("djs.list",{url:"",templateUrl:"modules/djs/client/views/list-djs.client.view.html"}).state("djs.create",{url:"/create",templateUrl:"modules/djs/client/views/create-dj.client.view.html",data:{roles:["user","admin"]}}).state("djs.view",{url:"/:djId",templateUrl:"modules/djs/client/views/view-dj.client.view.html"}).state("djs.edit",{url:"/:djId/edit",templateUrl:"modules/djs/client/views/edit-dj.client.view.html",data:{roles:["user","admin"]}})}]);
"use strict";angular.module("djs").controller("DjsController",["$scope","$stateParams","$location","Authentication","Djs","Programs","Shows",function($scope,$stateParams,$location,Authentication,Djs,Programs,Shows){$scope.authentication=Authentication,$scope.clear=function(){$scope.dj.title="",$scope.dj.image="",$scope.dj.images=[],$scope.dj.links={},$scope.dj.categories=[],$scope.dj.description={},$scope.dj.guest=!1,$scope.dj.featured=!1},$scope.create=function(isValid){if($scope.error=null,!isValid)return $scope.$broadcast("show-errors-check-validity","djForm"),!1;var dj=new Djs({title:$scope.dj.title,image:$scope.dj.image,images:$scope.dj.images,links:$scope.dj.links,categories:$scope.dj.categories,description:$scope.dj.description,guest:$scope.dj.guest,featured:$scope.dj.featured});dj.$save(function(response){$location.path("djs/"+response._id),$scope.clear()},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.remove=function(dj){if(dj){dj.$remove();for(var i in $scope.djs)$scope.djs[i]===dj&&$scope.djs.splice(i,1)}else $scope.dj.$remove(function(){$location.path("djs")})},$scope.update=function(isValid){if($scope.error=null,!isValid)return $scope.$broadcast("show-errors-check-validity","djForm"),!1;var dj=$scope.dj;dj.$update(function(){$location.path("djs/"+dj._id)},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.find=function(){$scope.djs=Djs.query({},function(data){var item=data[Math.floor(Math.random()*data.length)];$scope.changeBg(item.image)})},$scope.findOne=function(){$scope.dj=Djs.get({djId:$stateParams.djId},function(data){$scope.changeBg(data.image)}),$scope.programs=Programs.query({djId:$stateParams.djId,limit:5}),$scope.shows=Shows.query({djId:$stateParams.djId,limit:10})}}]);
"use strict";angular.module("djs").factory("Djs",["$resource",function($resource){return $resource("api/djs/:djId",{djId:"@_id"},{update:{method:"PUT"}})}]);
