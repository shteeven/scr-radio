...
"use strict";var app=angular.module("core");app.directive("scrCarousel",["$rootScope","$http","$interval",function($rootScope,$http,$interval){return{restrict:"E",scope:{slides:"="},controller:["$scope","$element",function($scope,$element){$scope.currentIndex=0;var rotationDelay=6800;$scope.isCurrentSlideIndex=function(index){return $scope.currentIndex===index},$scope.direction="slide-right",$scope.next=function(){$scope.direction="slide-left",$scope.currentIndex=$scope.currentIndex>0?--$scope.currentIndex:$scope.slides.length-1},$scope.previous=function(){$scope.direction="slide-right",$scope.currentIndex=$scope.currentIndex<$scope.slides.length-1?++$scope.currentIndex:0};var rotateSlide=$interval(function(){$scope.next()},rotationDelay);$scope.startRotation=function(){rotateSlide&&($interval.cancel(rotateSlide),rotateSlide=void 0),rotateSlide=$interval(function(){$scope.next()},rotationDelay)},$scope.stopRotation=function(){rotateSlide&&($interval.cancel(rotateSlide),rotateSlide=void 0)},$scope.$on("$destroy",function(){rotateSlide&&($interval.cancel(rotateSlide),rotateSlide=void 0)})}],templateUrl:"modules/core/client/views/components/carousel.html"}}]);
