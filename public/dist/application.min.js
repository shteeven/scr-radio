...
"use strict";var app=angular.module("core");app.directive("scrCarousel",["$rootScope","$http","$interval",function($rootScope,$http,$interval){return{restrict:"E",scope:{},controller:["$scope","$element",function($scope,$element){$http.get("modules/core/client/data/shows-list.json").then(function(data){$scope.slides=data.data}),$scope.slides=[],$scope.currentIndex=0,$scope.isCurrentSlideIndex=function(index){return $scope.currentIndex===index},$scope.direction="slide-right",$scope.next=function(){$scope.direction="slide-left",$scope.currentIndex=$scope.currentIndex>0?--$scope.currentIndex:$scope.slides.length-1},$scope.previous=function(){$scope.direction="slide-right",$scope.currentIndex=$scope.currentIndex<$scope.slides.length-1?++$scope.currentIndex:0};var rotateSlide=$interval(function(){$scope.next()},5e3);$scope.startRotation=function(){rotateSlide&&($interval.cancel(rotateSlide),rotateSlide=void 0),rotateSlide=$interval(function(){$scope.next()},5e3)},$scope.stopRotation=function(){rotateSlide&&($interval.cancel(rotateSlide),rotateSlide=void 0)},$scope.$on("$destroy",function(){rotateSlide&&($interval.cancel(rotateSlide),rotateSlide=void 0)})}],templateUrl:"modules/core/client/views/components/carousel.html"}}]);
...
"use strict";angular.module("things").run(["Menus",function(Menus){Menus.addMenuItem("topbar",{title:"Things",state:"things",type:"dropdown",roles:["*"]}),Menus.addSubMenuItem("topbar","things",{title:"Things",state:"things.list"}),Menus.addSubMenuItem("topbar","things",{title:"Create Things",state:"things.create",roles:["user"]})}]);
