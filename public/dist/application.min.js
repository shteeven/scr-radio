...
"use strict";angular.module("contents").controller("ContentsController",["$scope","$stateParams","$location","Authentication","Contents","$rootScope",function($scope,$stateParams,$location,Authentication,Contents,$rootScope){$scope.authentication=Authentication,$scope.tile_limit=120,$scope.rowLength=4,$scope.removeContent=function(content){if(content){content.$remove();for(var i in $scope.contents)$scope.contents[i]===content&&$scope.contents.splice(i,1)}else $scope.content.$remove(function(){$location.path("contents")})},$rootScope.changeBg($scope.defaultBg),$scope.find=function(){var query=$stateParams.contentType?{category:$stateParams.contentType}:{};$scope.contentType=$stateParams.contentType,$scope.contents=Contents.query(query,function(data){$scope.changeBg($scope.defaultBg)})},$scope.findOne=function(){console.log($stateParams.contentId),$scope.content=Contents.get({contentId:$stateParams.contentId},function(data){console.log(data),$scope.changeBg(data.image)})}}]);
...
"use strict";angular.module("core").directive("audioPlayer",["$rootScope","$document","$timeout","$http","$mdDialog","$mdMedia",function($rootScope,$document,$timeout,$http,$mdDialog,$mdMedia){return{restrict:"E",scope:{},link:function($scope,$element){function DialogController($scope,$mdDialog){$scope.hide=function(){$mdDialog.hide()},$scope.cancel=function(){$mdDialog.cancel()},$scope.answer=function(answer){$mdDialog.hide(answer)}}function getMetadata(){$http.jsonp("https://seoulcommunityradio.airtime.pro/api/live-info-v2",{params:{format:"jsonp",callback:"JSON_CALLBACK"}}).then(function(data){if(data=data.data,console.log(data.tracks),$scope.metadata&&data.tracks&&data.tracks.current&&data.tracks.current.name===$scope.metadata.tracks.current.name)timer=1e4,setTimer(timer);else if(data.tracks.current){$scope.metadata=data,timeNow=new Date(data.station.schedulerTime);var endDateTime=data.tracks.next?data.tracks.next.starts:data.tracks.current.ends;timeEnd=new Date(endDateTime),timer=timeEnd.getTime()-timeNow.getTime()+100,setTimer(timer)}else setTimer(resetTimer)},function(err){console.log(err)})}function setTimer(interval){$timeout.cancel(timeout),timeout=$timeout(function(){getMetadata()},interval)}DialogController.$inject=["$scope","$mdDialog"];var timeout,resetTimer=2e4;$scope.defaultText="All our Residents are currently getting their beauty sleep.";var timeNow,timeEnd,timer=resetTimer,radio=$document[0].createElement("audio");radio.src="http://seoulcommunityradio.out.airtime.pro:8000/seoulcommunityradio_a?1466567397161.mp3";var player={};player.isPlaying=!1,$scope.showLiveStream=function(ev){var useFullScreen=($mdMedia("sm")||$mdMedia("xs"))&&$scope.customFullscreen;$mdDialog.show({controller:DialogController,templateUrl:"/modules/contents/client/views/livestream.client.view.html",parent:angular.element(document.body),targetEvent:ev,clickOutsideToClose:!0,fullscreen:useFullScreen}).then(function(answer){$scope.status='You said the information was "'+answer+'".'},function(){$scope.status="You cancelled the dialog."}),$scope.$watch(function(){return $mdMedia("xs")||$mdMedia("sm")},function(wantsFullScreen){$scope.customFullscreen=wantsFullScreen===!0})},player.volumeChange=function(direction){"up"===direction&&radio.volume<1?(radio.volume=Math.round(100*(radio.volume+.1))/100,$scope.opacityPlus={opacity:1-.6*radio.volume},$scope.opacityMinus={opacity:.4+.6*radio.volume}):"down"===direction&&radio.volume>0&&(radio.volume=Math.round(100*(radio.volume-.1))/100,$scope.opacityPlus={opacity:1-.6*radio.volume},$scope.opacityMinus={opacity:.4+.6*radio.volume})},player.playPause=function(){player.isPlaying?(radio.pause(),$timeout.cancel(timeout),player.isPlaying=!1):(radio.play(),player.isPlaying=!0,$rootScope.$broadcast("audio.play",this))},$scope.$on("player.play",function(){radio.pause()}),$scope.player=player,$scope.radio=radio,radio.volume=.5,$scope.opacityPlus={opacity:1-.6*radio.volume},$scope.opacityMinus={opacity:.4+.6*radio.volume},setTimer(10),$scope.showLiveStream()},templateUrl:"modules/core/client/views/components/audio-player.html"}}]);
...
"use strict";angular.module("core").directive("scrHeader",function(){return{restrict:"E",scope:{},templateUrl:"modules/core/client/views/components/header.html",controllerAs:"ctrl",controller:["$rootScope",function($rootScope){var ctrl=this;ctrl.main_links={facebook:"https://www.facebook.com/seoulcommunityradio/",twitter:"https://twitter.com/radio_scr",mixcloud:"https://www.mixcloud.com/SCR_Radio/",instagram:"https://www.instagram.com/scr_radio/",soundcloud:"https://soundcloud.com/seoulcommunityradio"},ctrl.isCollapsed=!1,ctrl.toggleCollapsibleMenu=function(){ctrl.isCollapsed=!ctrl.isCollapsed||!1},$rootScope.$on("$stateChangeSuccess",function(){ctrl.isCollapsed=!1})}]}});
...
"use strict";angular.module("core").filter("jsonToHtml",function(){return function(input){return input?input.replace(/&amp;/,"&"):""}});
