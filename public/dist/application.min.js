...
"use strict";ApplicationConfiguration.registerModule("shows",["core"]),ApplicationConfiguration.registerModule("shows.admin",["core.admin"]),ApplicationConfiguration.registerModule("shows.admin.routes",["core.admin.routes"]);
...
"use strict";angular.module("core").controller("HomeController",["$scope","Authentication","Shows","Programs","Djs","$http",function($scope,Authentication,Shows,Programs,Djs,$http){$scope.authentication=Authentication,$scope.tile_limit=3,$scope.show_tiles=Shows.query(),$scope.program_tiles=Programs.query(),$scope.dj_tiles=Djs.query(),$http.get("api/getfeatured").then(function(data){console.log(data)})}]);
...
"use strict";angular.module("residents").config(["$stateProvider",function($stateProvider){$stateProvider.state("residents",{"abstract":!0,url:"/residents",template:'<div class="view"><ui-view/></div>'}).state("residents.list",{url:"",templateUrl:"modules/residents/client/views/list-residents.client.view.html"}).state("residents.create",{url:"/create",templateUrl:"modules/residents/client/views/create-dj.client.view.html",data:{roles:["user","admin"]}}).state("residents.view",{url:"/:djId",templateUrl:"modules/residents/client/views/view-dj.client.view.html"}).state("residents.edit",{url:"/:djId/edit",templateUrl:"modules/residents/client/views/edit-dj.client.view.html",data:{roles:["user","admin"]}})}]);
"use strict";angular.module("residents").controller("ResidentsController",["$scope","$stateParams","$location","Authentication","Residents","Programs","Shows",function($scope,$stateParams,$location,Authentication,Residents,Programs,Shows){$scope.authentication=Authentication,$scope.clear=function(){$scope.dj.title="",$scope.dj.image="",$scope.dj.images=[],$scope.dj.links={},$scope.dj.categories=[],$scope.dj.description={},$scope.dj.guest=!1,$scope.dj.featured=!1},$scope.create=function(isValid){if($scope.error=null,!isValid)return $scope.$broadcast("show-errors-check-validity","djForm"),!1;var dj=new Residents({title:$scope.dj.title,image:$scope.dj.image,images:$scope.dj.images,links:$scope.dj.links,categories:$scope.dj.categories,description:$scope.dj.description,guest:$scope.dj.guest,featured:$scope.dj.featured});dj.$save(function(response){$location.path("residents/"+response._id),$scope.clear()},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.remove=function(dj){if(dj){dj.$remove();for(var i in $scope.residents)$scope.residents[i]===dj&&$scope.residents.splice(i,1)}else $scope.dj.$remove(function(){$location.path("residents")})},$scope.update=function(isValid){if($scope.error=null,!isValid)return $scope.$broadcast("show-errors-check-validity","djForm"),!1;var dj=$scope.dj;dj.$update(function(){$location.path("residents/"+dj._id)},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.find=function(){$scope.residents=Residents.query({},function(data){var item=data[Math.floor(Math.random()*data.length)];$scope.changeBg(item.image)})},$scope.findOne=function(){$scope.dj=Residents.get({djId:$stateParams.djId},function(data){$scope.changeBg(data.image)}),$scope.programs=Programs.query({djId:$stateParams.djId,limit:5}),$scope.shows=Shows.query({djId:$stateParams.djId,limit:10})}}]);
"use strict";angular.module("residents").factory("Residents",["$resource",function($resource){return $resource("api/residents/:djId",{djId:"@_id"},{update:{method:"PUT"}})}]);
"use strict";angular.module("programs").run(["Menus",function(Menus){Menus.addMenuItem("topbar",{title:"Programs",state:"programs",type:"dropdown",roles:["*"]}),Menus.addSubMenuItem("topbar","programs",{title:"List Programs",state:"programs.list"}),Menus.addSubMenuItem("topbar","programs",{title:"Create Programs",state:"programs.create",roles:["user"]})}]);
...
"use strict";angular.module("shows").run(["Menus",function(Menus){Menus.addMenuItem("topbar",{title:"Shows",state:"shows",type:"dropdown",roles:["*"]}),Menus.addSubMenuItem("topbar","shows",{title:"List Shows",state:"shows.list"}),Menus.addSubMenuItem("topbar","shows",{title:"Create Shows",state:"shows.create",roles:["user"]})}]);
"use strict";angular.module("shows").config(["$stateProvider",function($stateProvider){$stateProvider.state("shows",{"abstract":!0,url:"/shows",template:'<div class="view"><ui-view/></div>'}).state("shows.list",{url:"",templateUrl:"modules/shows/client/views/list-shows.client.view.html"}).state("shows.create",{url:"/create",templateUrl:"modules/shows/client/views/create-show.client.view.html",data:{roles:["user","admin"]}}).state("shows.view",{url:"/:showId",templateUrl:"modules/shows/client/views/view-show.client.view.html"}).state("shows.edit",{url:"/:showId/edit",templateUrl:"modules/shows/client/views/edit-show.client.view.html",data:{roles:["user","admin"]}})}]);
"use strict";angular.module("shows").controller("ShowsController",["$scope","$stateParams","$location","Authentication","Shows","Djs","Programs",function($scope,$stateParams,$location,Authentication,Shows,Djs,Programs){$scope.authentication=Authentication,$scope.days=["monday","tuesday","wednesday","thursday","friday","saturday","sunday"],$scope.getDjs=function(){$scope.djs=Djs.query()},$scope.getPrograms=function(){$scope.programs=Programs.query()},$scope.addDj=function(dj){$scope.show.djs.indexOf(dj)<0&&$scope.show.djs.push(dj)},$scope.removeDj=function(dj){$scope.show.djs=$scope.show.djs.filter(function(obj){return obj!==dj})},$scope.getDjName=function(dj){var dj_obj=$scope.djs.filter(function(obj){return obj._id===dj});return dj_obj[0].title},$scope.clear=function(){$scope.show={},$scope.show.title="",$scope.show.image="",$scope.show.images=[],$scope.show.links={},$scope.show.categories=[],$scope.show.description={},$scope.show.djs=[],$scope.show.guests=[],$scope.show.aired=new Date,$scope.program="",$scope.show.featured=!1},$scope.create=function(isValid){if($scope.error=null,!isValid)return $scope.$broadcast("show-errors-check-validity","showForm"),!1;var show=new Shows({title:$scope.show.title,image:$scope.show.image,images:$scope.show.images,links:$scope.show.links,categories:$scope.show.categories,description:$scope.show.description,djs:$scope.show.djs,guests:$scope.show.guests,aired:$scope.show.aired,program:$scope.program,featured:$scope.show.featured});show.$save(function(response){$location.path("shows/"+response._id),$scope.clear()},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.remove=function(show){if(show){show.$remove();for(var i in $scope.shows)$scope.shows[i]===show&&$scope.shows.splice(i,1)}else $scope.show.$remove(function(){$location.path("shows")})},$scope.update=function(isValid){if($scope.error=null,!isValid)return $scope.$broadcast("show-errors-check-validity","showForm"),!1;var show=$scope.show;show.$update(function(){$location.path("shows/"+show._id)},function(errorResponse){$scope.error=errorResponse.data.message})},$scope.find=function(){$scope.shows=Shows.query({},function(data){var item=data[Math.floor(Math.random()*data.length)];$scope.changeBg(item.image)})},$scope.findOne=function(){$scope.show=Shows.get({showId:$stateParams.showId},function(data){$scope.changeBg(data.image)})}}]);
"use strict";angular.module("shows").factory("Shows",["$resource",function($resource){return $resource("api/shows/:showId",{showId:"@_id"},{update:{method:"PUT"}})}]);
